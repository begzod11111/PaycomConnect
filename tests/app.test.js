...
const { resetMemoryStore } = await import('../services/persistenceService.js');
...
test('slack connect command completes link and normal telegram message is forwarded only after link', async () => {
  await request(app).post('/api/mock/telegram').send({
    messageId: 'tg-connect-1',
    userId: '1001',
    userName: 'Manager Ali',
    channelId: '-100777',
  ...
  const slackConnect = await request(app)
  ...
  const tgMessage = await request(app)
    .post('/api/mock/telegram')
    .send({
      messageId: 'tg-msg-1',
      userId: '1002',
...
test('message is not forwarded before link is completed', async () => {
  ...
  const response = await request(app).post('/api/mock/telegram').send({
...
test('duplicate message is not counted twice after link', async () => {
  ...
  const first = await request(app).post('/api/mock/slack').send(payload);
  const duplicate = await request(app).post('/api/mock/slack').send(payload);
  const summary = await request(app).get('/api/analytics/summary');
